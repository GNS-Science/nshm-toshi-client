...
from .file_details_query import FileDetailsQuery
from .one_general import OneGeneral
...
class Client(AsyncBaseClient):
    async def file_details_query(self, id: str, **kwargs: Any) -> FileDetailsQuery:
        query = gql(
            """
        ...
        return FileDetailsQuery.model_validate(data)

    async def one_general(self, id: str, **kwargs: Any) -> OneGeneral:
        query = gql(
            """
        ...
        variables: Dict[str, object] = {"id": id}
        response = await self.execute(
            query=query, operation_name="one_general", variables=variables, **kwargs
        )
        data = self.get_data(response)
        return OneGeneral.model_validate(data)

    async def create_general_task(
        self, created: Any, agent_name: str, title: str, description: str, **kwargs: Any
    ) -> CreateGeneralTask:
        query = gql(
            """
            mutation CreateGeneralTask($created: DateTime!, $agent_name: String!, $title: String!, $description: String!) {
              create_general_task(
                input: {created: $created, agent_name: $agent_name, title: $title, description: $description}
              ) {
                general_task {
                  id
                }
